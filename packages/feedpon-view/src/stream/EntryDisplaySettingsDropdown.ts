...
export const EntryDisplaySettingsDropdown = createComponent(
  function EntryDisplaySettingsDropdown(
    {
      activeEntryIndex,
      canMarkStreamAsRead,
      onClearReadPosition,
    ...
    const handleExpandBehind = $.useCallback((event: Event) => {
    ...
    const handleExpandAhead = $.useCallback((event: Event) => {
      event.preventDefault();
      setDropdownState((state) => ({
        ...state,
        aheadExpanded: true,
    ...
    const handleScrollToUnreadPosition = $.useCallback(() => {
      onScrollToEntry(activeEntryIndex + 1);
    }, [onScrollToEntry, activeEntryIndex]);

    const handleClearReadPosition = $.useCallback(() => {
      onClearReadPosition();
    }, [onClearReadPosition]);
