...
export default function connect<TStateProps, TDispatchProps, TOwnProps>(
  connection:
    | Connection<TStateProps, TDispatchProps, TOwnProps>
    | (() => Connection<TStateProps, TDispatchProps, TOwnProps>),
) {
  return <
    TProps extends TStateProps &
      TDispatchProps &
      TOwnProps & { children?: React.ReactNode },
  >(
    WrappedComponent: React.ComponentType<TProps>,
  ): React.ComponentType<Partial<TProps>> => {
    return class StoreSubscriber extends PureComponent<
      Partial<TProps>,
      TStateProps
    > {
      private readonly _renderer: (
        context: StoreContext,
        ownProps: Partial<TProps>,
      ) => React.ReactNode;
      ...
      constructor(props: Partial<TProps>) {
        ...
        this._renderer = createSelector(propsSelector, (props) =>
          createElement(WrappedComponent, props, props.children),
        );
      }

      override render() {
        return (
          <StoreContext.Consumer>{this._renderChild}</StoreContext.Consumer>
        );
      }
