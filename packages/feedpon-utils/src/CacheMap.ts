...
export function map<T, TResult>(
  map: CacheMap<T>,
  keySelector: (value: T, key: string, index: number) => string,
  valueSelector: (value: T, key: string, index: number) => TResult,
) {
...
export function mapValues<T, TResult>(
  map: CacheMap<T>,
  selector: (value: T, key: string, index: number) => TResult,
): CacheMap<TResult> {
  return map.keys.reduce((acc, k, i) => {
    const v = map.indices[k]!;
    acc.keys.push(k);
...
export function filter<T>(
  map: CacheMap<T>,
  predicate: (element: T, key: string, index: number) => boolean,
): CacheMap<T> {
  return map.keys.reduce((acc, k, i) => {
