...
export default function parseDuration(input: string): number {
  ...
  const h = matches[1] ? Number.parseInt(matches[1], 10) : 0;
  const m = matches[2] ? Number.parseInt(matches[2], 10) : 0;
  const s = matches[3] ? Number.parseInt(matches[3], 10) : 0;
  const ms = matches[4] ? Number.parseInt(matches[4], 10) : 0;

  return (
    h * MILLISECONDS_OF_HOUR +
