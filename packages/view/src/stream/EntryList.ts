import type { Entry, Session, Stream } from '@feedpon/model';
import {
  throttle,
  type VirtualRange,
  VirtualScroller,
  type VirtualScrollerHandle,
} from '@feedpon/primitives';
import { createComponent, html, type Ref } from 'barebind';
...
export const EntryList = createComponent<EntryListProps>(function EntryList({
  ...
  const scrollCallback = this.use(
    EffectEvent(() => {
      ...
      const virtualRange = virtualScroller.getVirtualRange();
      const focusIndex = getFocusIndex(visibleElements, virtualRange);
  ...
  const virtualScroller = VirtualScroller({
    ...
    onVirtualRangeChange: throttledScrollCallback,
...
function getFocusIndex(
  ...
  if (mostVisibleIndex >= 0) {
    return virtualRange.start + mostVisibleIndex;
  }

  if (traversedIndex >= 0) {
    return virtualRange.end;
  }
