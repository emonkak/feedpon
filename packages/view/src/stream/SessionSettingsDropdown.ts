...
export const SessionSettingsDropdown =
  createComponent<SessionSetttingsDropdownProps>(function SessionDropdown({
    sessionSettings,
    disabled,
    onSessionSettingsUpdate,
  }) {
    const count$ = this.use(LocalAtom(sessionSettings.count));
    const layout$ = this.use(LocalAtom(sessionSettings.layout));
    const ranked$ = this.use(LocalAtom(sessionSettings.ranked));
    const unreadOnly$ = this.use(LocalAtom(sessionSettings.unreadOnly));
    const sessionSettings$ = this.use(
      LocalComputed(
    ...
    const handleRankedChange = (_event: Event, key: string) => {
      ranked$.value = key as EntryRanking;
      onSessionSettingsUpdate(sessionSettings$.value, sessionSettings);
    };
    ...
    const items: MenuItem[] = [
      ...
      },
      {
        ...
        childItems: [
          {
            key: 'engagement' as EntryRanking,
            label: 'Higher engagement first',
          },
          { key: 'newest' as EntryRanking, label: 'Newest first' },
          { key: 'oldest' as EntryRanking, label: 'Oldest first' },
        ].map(
          ({ key, label }) =>
            ({
              type: 'button',
