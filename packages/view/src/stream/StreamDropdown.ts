import type { Session, Stream } from '@feedpon/model';
...
export const StreamDropdown = createComponent<StreamDropdownProps>(
  function StreamDropdown({
    ...
    const [dropdownState, setDropdownState] = this.useState({
      aheadExpanded: false,
      behindExpanded: false,
    });

    const range = this.useMemo(() => {
    ...
    const totalReadEntries =
      Math.max(session.focusIndex, session.readIndex) + 1;
    ...
    const handleStreamMarkAsRead = this.useCallback(() => {
      openAlertDialog({
    ...
    const handleScrollToEntry = this.useCallback(
