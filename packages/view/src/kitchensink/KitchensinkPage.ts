...
import { AppStore, type NotificationType } from '@feedpon/model';
import { sendNotification, toggleSidebar } from '@feedpon/model/actions/ui';
import { Dropdown, Modal, Navbar } from '@feedpon/primitives';
import { createComponent, html } from 'barebind';
import { MainLayout } from '../layout/MainLayout.ts';

export interface KitchenSinkProps {}

export const KitchensinkPage = createComponent<KitchenSinkProps>(
  function KitchensinkPage() {
    const { onNotificationSend, onSidebarToggle } = this.use(
      BindActionCreators(AppStore, {
        onNotificationSend: sendNotification,
        onSidebarToggle: toggleSidebar,
      }),
    );
    const [modalOpened, setModalOpened] = this.useState(false);

    const handleOpenModal = this.useCallback(() => {
      setModalOpened(true);
    }, []);

    const handleCloseModal = this.useCallback(() => {
      setModalOpened(false);
    }, []);

    const handleSendNotification = this.useCallback(
    ...
    const content = html`
      <div class="container">
            ...
            items: [
              {
                ...
                `,
                type: 'button',
                onAction: this.useCallback(() => alert('First'), []),
                key: 'first',
              },
              {
                children: html`
                ...
                `,
                type: 'button',
                onAction: this.useCallback(() => alert('Second'), []),
                key: 'second',
              },
              {
                key: 'separator1',
              ...
              },
              {
                ...
                `,
                type: 'button',
                onAction: this.useCallback(() => alert('Thrid'), []),
                key: 'thrid',
              },
            ],
          })}>
        ...
        <p>
          ...
          <i class="icon icon-16 icon-feedly"></i>
