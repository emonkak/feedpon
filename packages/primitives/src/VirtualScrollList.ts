...
import { IsMounted } from './hooks/IsMounted.ts';
import { PreviousValue } from './hooks/PreviousValue.ts';
...
export const VirtualScrollList: VirtualScrollList = createComponent(
  function VirtualScrollList<TItem extends { id: PropertyKey }>(
    ...
    const oldItems = this.use(PreviousValue(items)) ?? [];
    if (items !== oldItems) {
    ...
    const isMounted = this.use(IsMounted());
