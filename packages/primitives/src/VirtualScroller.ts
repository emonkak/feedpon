...
export const VirtualScroller: VirtualScroller = createComponent(
  function VirtualScroller<TSource, TKey, TElement>(
    ...
    const [virtualRange, setVirtualRange] = this.useState<VirtualRange>({
      start: 0,
      end: 0,
    });
    ...
    const computeVirtualRange = (top: number, bottom: number): VirtualRange => {
      ...
      IntersectionObserver(
        (entries) => {
          for (const entry of entries) {
            ...
            const virtualRange = computeVirtualRange(top, bottom);

            setVirtualRange(virtualRange, {
    ...
    this.use(
      ImperativeHandle(ref, () => ({
        ...
        async scrollToIndex(
          ...
          if (!withinRange(virtualRange, index)) {
            const virtualRange = {
              start: index,
              end: index + 1,
            };
            intersectionObserver.disconnect();
            await setVirtualRange(virtualRange).finished;
            onVirtualRangeChange?.(virtualRange);
          }
    ...
    const headSpace = computeRangeHeight(0, virtualRange.start);
    const tailSpace = computeRangeHeight(virtualRange.end, source.length);
        ...
        <ul class="VirtualScroller-list">
          <${source
            ...
            .map((item, offset) => {
              ...
              `.withKey(keySelector(item, virtualRange.start + offset));
            })}>
        </ul>
...
function areRangesEqual(x: VirtualRange, y: VirtualRange) {
  return x.start === y.start && x.end === y.end;
}

function withinRange(range: VirtualRange, index: number): boolean {
