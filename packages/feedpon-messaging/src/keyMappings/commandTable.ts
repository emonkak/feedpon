  ...
  action() {
    return ({ dispatch }) => {
      ...
      if (selectedStream && selectedStream.streamView === 'collapsible') {
        dispatch(
          uiActions.changeExpandedEntry(
            selectedStream.streamId,
            selectedStream.activeEntryIndex,
          ),
        );
      }
    };
  ...
  action({ template, inBackground }) {
    return ({ dispatch }) => {
      ...
      if (entry && entry.url) {
        ...
        const url = template.replace(TEMPLATE_PATTERN, (_match, p1) => {
  ...
  action() {
    return ({ getState }, { router, selectors }) => {
      ...
      const groupedSubscriptions =
        selectors.groupedSubscriptionsSelector(state);
      ...
      if (streamId) {
        const selectedIndex = visibleCategories.findIndex(
          (category) =>
            category.streamId === streamId ||
            (groupedSubscriptions[category.label] &&
              groupedSubscriptions[category.label]!.items.findIndex(
                (subscription) => subscription.streamId === streamId,
              ) !== -1),
        );
  ...
  action() {
    return ({ getState }, { router, selectors }) => {
      ...
      const visibleSubscriptions =
        selectors.visibleSubscriptionsSelector(state);
      ...
      if (streamId) {
        ...
        if (selectedCategory) {
          targetIndex = visibleSubscriptions.findIndex(
            (subscription) =>
              subscription.labels.indexOf(selectedCategory.label) !== -1,
          );
        } else {
          const selectedIndex = visibleSubscriptions.findIndex(
            (subscription) => subscription.streamId === streamId,
          );
          targetIndex = selectedIndex > -1 ? selectedIndex + 1 : 0;
        }
    ...
    return ({ getState }, { router, selectors }) => {
      ...
      if (streamId) {
        ...
        if (selectedIndex > -1) {
          ...
          targetIndex =
            selectedCategory.streamId === streamId
              ? selectedIndex - 1
              : selectedIndex;
        } else {
  ...
  action() {
    return ({ getState }, { router, selectors }) => {
      ...
      if (streamId) {
        ...
        if (selectedCategory) {
          const firstIndex = visibleSubscriptions.findIndex(
            (subscription) =>
              subscription.labels.indexOf(selectedCategory.label) !== -1,
          );
          targetIndex =
            firstIndex > -1 ? firstIndex - 1 : visibleSubscriptions.length - 1;
        } else {
          const selectedIndex = visibleSubscriptions.findIndex(
            (subscription) => subscription.streamId === streamId,
          );
...
export const toggleStreamView: Command<{}> = {
  ...
  description:
    'Toggle between expanded stream view and collapsible stream view.',
  defaultParams: {},
  action() {
    return ({ dispatch }) => {
      const selectedStream = dispatch(getSelectedStream);
      ...
      if (selectedStream) {
        const newStreamView =
          selectedStream.streamView === 'collapsible'
            ? 'expanded'
            : 'collapsible';

        dispatch(
          uiActions.changeStreamView(selectedStream.streamId, newStreamView),
        );
      }
    };
  },
...
const getActiveEntry: Thunk<Entry | null> = ({ dispatch }) => {
  const selectedStream = dispatch(getSelectedStream);
