        ...
        items: CacheMap.mapValues(streams.items, (stream) => ({
          ...
          entries: stream.entries.map((entry) => {
            if (
              !(
                entry.isPinning ||
                entry.comments.isLoading ||
                entry.fullContents.isLoading
              )
            ) {
              return entry;
            }
          ...
          activeEntryIndex:
            stream.activeEntryIndex != null ? stream.activeEntryIndex : -1,
          expandedEntryIndex:
            stream.expandedEntryIndex != null ? stream.expandedEntryIndex : -1,
          readEntryIndex:
            stream.readEntryIndex != null ? stream.readEntryIndex : -1,
          streamView:
            stream.streamView != null
              ? stream.streamView
              : streams.defaultStreamView,
        })),
      ...
      return {
        ...
        items: CacheMap.update(streams.items, event.streamId, (stream) => {
          return {
            ...
            readEntryIndex:
              event.index > stream.readEntryIndex
                ? event.index - 1
                : stream.readEntryIndex,
          };
        }),
      };
    ...
    case 'FEED_SUBSCRIBED':
      return {
        ...
        items: CacheMap.mapValues(streams.items, (stream) => {
          if (
            !stream.feed ||
            stream.feed.feedId !== event.subscription.feedId
          ) {
            return stream;
          }
          return {
