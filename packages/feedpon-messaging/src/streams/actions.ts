    ...
    try {
      if (streamId.startsWith('feed/')) {
        stream = await dispatch(
          fetchFeedStream(streamId, streamView, fetchOptions, fetchedAt),
        );
      } else if (streamId.startsWith('user/')) {
        stream = await dispatch(
          fetchCategoryStream(streamId, streamView, fetchOptions, fetchedAt),
        );
      } else if (streamId === ALL_STREAM_ID) {
        stream = await dispatch(
          fetchAllStream(streamView, fetchOptions, fetchedAt),
        );
      } else if (streamId === PINS_STREAM_ID) {
        stream = await dispatch(
          fetchPinsStream(streamView, fetchOptions, fetchedAt),
        );
      }
    } catch (error) {
      dispatch({
...
export function fetchEntryComments(
...
export function markAsRead(entries: Entry[]): AsyncThunk {
  return async ({ dispatch }, { environment }) => {
    ...
    try {
      ...
      dispatch(sendNotification(message, 'positive'));
    } catch (error) {
      dispatch({
...
export function markAllAsRead(): AsyncThunk {
  return async ({ dispatch, getState }, { environment }) => {
    ...
    try {
      ...
      const numEntries = Object.values(subscriptions).reduce(
  ...
  return async ({ dispatch, getState }, { environment }) => {
    ...
    try {
      ...
      const numEntries = Object.values(subscriptions.items).reduce(
        (total, subscription) =>
          total +
          (subscription.labels.includes(category.label)
...
export function changeDefaultStreamFetchOptions(
  fetchOptions: StreamFetchOptions,
): AsyncThunk {
  return async ({ dispatch }) => {
    dispatch({
    ...
    dispatch(
      sendNotification('Default stream fetch options changed', 'positive'),
    );
  };
}

export function changeDefaultStreamView(
  streamView: StreamViewKind,
): AsyncThunk {
  return async ({ dispatch }) => {
    dispatch({
    ...
    dispatch(sendNotification('Default stream view changed', 'positive'));
  };
}
...
export function changeStreamCacheCapacity(capacity: number): Thunk {
  return ({ dispatch }) => {
    ...
    dispatch(sendNotification('Stream cache capacity changed', 'positive'));
  };
}
...
export function changeStreamCacheLifetime(lifetime: number): Thunk {
  return async ({ dispatch }) => {
    ...
    dispatch(sendNotification('Stream cache lifetime changed', 'positive'));
  };
}
...
export function changeStreamHistoryOptions(
  ...
  return async ({ dispatch }) => {
    ...
    dispatch(sendNotification('Stream history options changed', 'positive'));
  };
}

function fetchFeedStream(
  streamId: string,
  streamView: StreamViewKind,
  fetchOptions: StreamFetchOptions,
  fetchedAt: number,
...
function fetchBookmarkCounts(urls: string[]): AsyncThunk {
  return async ({ dispatch }) => {
    if (urls.length > 0) {
      const bookmarkCounts = await hatena.getBookmarkCounts(urls);
...
function expandUrls(urls: string[]): AsyncThunk<string[]> {
  return async ({ dispatch, getState }) => {
    ...
    const matchedUrls = urls.filter((url) =>
      trackingUrls.patterns.some((pattern) => tryMatch(pattern, url)),
    );
    ...
    const expandedUrls = results.reduce<{ [key: string]: string }>(
...
function getEntryConverter(): Thunk<(entry: feedly.Entry) => Entry> {
  return ({ dispatch }) => {
    ...
    return (entry) => ({
      ...
      bookmarkCount: 0,
      isPinned: entry.tags
        ? entry.tags.some((tag) => tag.id.endsWith('tag/global.saved'))
        : false,
      isPinning: false,
...
function getUrlReplacer(): Thunk<(url: string) => string> {
  return ({ getState }) => {
    ...
    return urlReplacements.items.reduce<(url: string) => string>(
      (func, item) => {
        try {
