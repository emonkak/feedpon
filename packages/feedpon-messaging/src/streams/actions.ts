...
import type { AsyncThunk, Category, Entry, Event, Stream, StreamFetchOptions, StreamViewKind, Subscription, Thunk } from '../index';
...
export function markFeedAsRead(subscription: Subscription): AsyncThunk {
    return async ({ dispatch }) => {
        ...
        try {
            ...
            await feedly.markAsReadForFeeds(token.access_token, subscription.feedId as string);
...
export function markCategoryAsRead(category: Category): AsyncThunk {
    return async ({ dispatch, getState }) => {
        ...
        try {
            ...
            await feedly.markAsReadForCategories(token.access_token, category.categoryId as string);
...
function extractContent(contentDocument: Document, _url: string, contentExpression: string): string {
    let content = '';
