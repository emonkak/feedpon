...
import type { Event, Subscription, Subscriptions } from '../types.ts';
  ...
  switch (event.type) {
    ...
    case 'SUBSCRIPTIONS_FETCHED':
      return {
        ...subscriptions,
        isLoading: false,
        items: event.subscriptions.reduce<{
    ...
    case 'CATEGORY_UPDATED':
      return {
        ...
        items: mapObject(subscriptions.items, (subscription) => {
          const labels = subscription.labels.filter(
            (label) => label !== event.prevCategory.label,
          );
      ...
      return {
        ...
        items: mapObject(subscriptions.items, (subscription) => {
          ...
          return {
            ...subscription,
            readCount:
              subscription.readCount + event.readCounts[subscription.streamId]!,
          };
        }),
      };
