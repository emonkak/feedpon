...
export function createGroupedSubscriptionsSelector(
  ...
  return createSelector(subscriptionsSelector, getGroupedSubscriptions);
}

export function createTotalUnreadCountSelector(
  subscriptionsSelector: (state: State) => Subscription[],
) {
  return createSelector(subscriptionsSelector, getTotalUnreadCount);
}

function getAllSubscriptions(subscriptions: {
...
function getVisibleSubscriptions(
  ...
  if (onlyUnread) {
    ...
    return Object.values(subscriptions).sort(comparer);
  }
}

function getGroupedSubscriptions(subscriptions: Subscription[]): {
  [key: string]: GroupedSubscription;
} {
  return subscriptions.reduce<{ [key: string]: GroupedSubscription }>(
    (acc, subscription) => {
      const labels =
        subscription.labels.length > 0 ? subscription.labels : [UNCATEGORIZED];
      for (const label of labels) {
        if (acc[label]) {
          acc[label]!.items.push(subscription);
          if (subscription.unreadCount > subscription.readCount) {
            acc[label]!.unreadCount +=
