import type { Patch } from '@feedpon/foundation';
import type { AppStateRepository, Feed, Stream } from '@feedpon/model';
...
export class IndexedDBStateRepository implements AppStateRepository {
  ...
  deleteAllFeeds(): Promise<void> {
    return this._runInTransaction(
      async (feedsStore) => {
        await iterateCursor(feedsStore.openCursor(), (cursor) => {
          cursor.delete();
          cursor.continue();
        });
      },
  ...
  deleteAllStreams(): Promise<void> {
    return this._runInTransaction(
      async (streamsStore) => {
        await iterateCursor(streamsStore.openCursor(), (cursor) => {
          cursor.delete();
          cursor.continue();
        });
      },
